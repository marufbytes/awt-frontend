...
export default function ManageCompaniesPage() {
  ...
  const loadData = () => {
    ...
    fetch('http://localhost:3000/company', { credentials: 'include' })
      .then(res => res.json())
      .then(data => {
        setList(Array.isArray(data) ? data : (data.companies || data.data || []));
        setLoading(false);
      })
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    const res = await fetch(`http://localhost:3000/company/${id}`, { method: 'DELETE', credentials: 'include' });
  ...
  const filtered = list.filter(item => {
    ...
    const matchStatus = 
      ...
      statusFilter === 'Verified' ? item.isVerified : !item.isVerified;
      ...
      <div className="bg-white rounded-xl shadow-xs overflow-hidden">
        {loading ? (
          ...
          <table className="table w-full">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={4} className="text-center py-6 text-slate-400">No companies found</td></tr>
              ) : (
                filtered.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="text-slate-600 text-xs">{item.industry || 'N/A'}</td>
                    <td>
                      <span className={`badge badge-sm font-semibold ${item.isVerified ? 'badge-success text-white' : 'badge-warning text-white'}`}>
                    ...
                    <td className="text-right space-x-1">
                      ...
                      <button onClick={() => handleDelete(item.id, item.name)} className="btn btn-ghost btn-xs text-red-500">
      ...
      <div className={`modal ${selected ? 'modal-open' : ''}`}>
        <div className="modal-box rounded-2xl space-y-4 max-w-md">
          <div className="flex justify-between items-center border-b pb-3">
            <h3 className="font-extrabold text-lg text-slate-900">{selected?.name}</h3>
            <button onClick={() => setSelected(null)} className="btn btn-sm btn-circle btn-ghost">✕</button>
          </div>
          {selected && (
            <div className="text-xs space-y-2 text-slate-600">
              <p><strong className="text-slate-800">Industry:</strong> {selected.industry || 'N/A'}</p>
              <p><strong className="text-slate-800">Email:</strong> {selected.email || 'N/A'}</p>
              <p><strong className="text-slate-800">Phone:</strong> {selected.phone || 'N/A'}</p>
              <p><strong className="text-slate-800">Location:</strong> {selected.location || 'N/A'}</p>
              <p><strong className="text-slate-800">Description:</strong> {selected.description || 'N/A'}</p>
              <div className="pt-2 border-t">
                <strong className="text-slate-800 block mb-1">Associated Internships ({selected.internships?.length || 0}):</strong>
                {selected.internships?.length > 0 ? (
                  <ul className="list-disc pl-4 space-y-1">
                    {selected.internships.map((int: any) => (
                      <li key={int.id}>
                        {int.title} <span className="text-slate-400">({int.isActive ? 'Active' : 'Closed'})</span>
                      </li>
                    ))}
                  </ul>
                ) : (
